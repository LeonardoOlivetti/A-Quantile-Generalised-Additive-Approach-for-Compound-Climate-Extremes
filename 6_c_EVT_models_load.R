...
ggsave(file = "Qgam wind bias.pdf",
       device="pdf",
       path="Pics",
...
ggsave(file = "Qgam prec bias.pdf",
       device="pdf",
       path="Pics",
       width = 12, # The width of the plot in inches
       height = 7.5,
...
ggsave(file = "Qgam Qreg wind R2.pdf",
       device="pdf",
       path="Pics",
       width = 12, # The width of the plot in inches
       height = 7.5,
...
ggsave(file = "Qgam Qreg prec R2.pdf",
       device="pdf",
       path="Pics",
       width = 12, # The width of the plot in inches
       height = 7.5,
...
ggsave(file = "Qgam Pot wind R2.pdf",
       device="pdf",
       path="Pics",
       width = 12, # The width of the plot in inches
       height = 7.5,
...
ggsave(file = "Qgam Pot prec R2.pdf",
       device="pdf",
       path="Pics",
       width = 12, # The width of the plot in inches
       height = 7.5,
...
logLik(q_jet_wind_95)
logLik(q_base_wind_95)

############POSTER EXTRA##################

library(evmix)

da<-POT_declust_jet(tot_train,wind,0.9,over=TRUE)
da$dat$excesses<-da$dat$key_var-da$threshold
...
POT_jet_wind_99<-evgam(list(excesses~as.numeric(lat)*as.numeric(lon)+month+as.numeric(year)+as.numeric(US_temp_lag2)+as.numeric(jet_strength_lag1)+as.numeric(lat_jet_lag1)+as.numeric(NAO_lag1)+as.numeric(jet_proximity_lag1)+as.numeric(qu_99_wind),~1),family="gpd",da$dat)
R2_POT_jet_wind_99<-Pseudo_R2_point_EVT(POT_jet_wind_99,wind,0.99,0.9,tot_train,tot_test)

R2_multi_POT_jet_R2_99<-Pseudo_R2_point_multi_EVT(pred_q_jet_wind_99,POT_jet_wind_99,wind,0.99,0.9,tot_train,tot_test)
p_q_jet_99_R2_wind<-Plot_R2(R2_multi_POT_jet_R2_99,0.99,up_lim = 0.11, low_lim = -0.11)

POT_jet_wind_95<-evgam(list(excesses~as.numeric(lat)*as.numeric(lon)+month+as.numeric(year)+as.numeric(US_temp_lag2)+as.numeric(jet_strength_lag1)+as.numeric(lat_jet_lag1)+as.numeric(NAO_lag1)+as.numeric(jet_proximity_lag1)+as.numeric(qu_95_wind),~1),family="gpd",da$dat)
R2_POT_jet_wind_95<-Pseudo_R2_point_EVT(POT_jet_wind_95,wind,0.95,0.5,tot_train,tot_test)
p_POT_jet_95_R2<-Plot_R2(R2_POT_jet_wind_95,0.95,up_lim = 0.62)
Pseudo_R2_jet_95_QGAM_POT_wind<-Pseudo_R2_tot_multi_EVT(pred_q_jet_wind_95,POT_jet_wind_95,wind,0.95,0.5,tot_train,tot_test)

R2_multi_POT_jet_R2_95<-Pseudo_R2_point_multi_EVT(pred_q_jet_wind_95,POT_jet_wind_95,wind,0.95,0.5,tot_train,tot_test)
p_q_jet_95_R2_wind<-Plot_R2(R2_multi_POT_jet_R2_95,0.95,up_lim = 0.11, low_lim = -0.11)

da<-POT_declust_jet(tot_train,prec,0.9,over=TRUE)
da$dat$excesses<-da$dat$key_var-da$threshold
...
POT_jet_prec_99<-evgam(list(excesses~as.numeric(lat)*as.numeric(lon)+month+as.numeric(year)+as.numeric(US_temp_lag2)+as.numeric(jet_strength_lag1)+as.numeric(lat_jet_lag1)+as.numeric(NAO_lag1)+as.numeric(jet_proximity_lag1)+as.numeric(qu_99_prec),~1),family="gpd",da$dat)
R2_POT_jet_prec_99<-Pseudo_R2_point_EVT(POT_jet_prec_99,prec,0.99,0.9,tot_train,tot_test)

R2_multi_POT_jet_R2_99_prec<-Pseudo_R2_point_multi_EVT(pred_q_jet_prec_99,POT_jet_prec_99,prec,0.99,0.9,tot_train,tot_test)
p_q_jet_99_R2_prec<-Plot_R2(R2_multi_POT_jet_R2_99,0.99,up_lim = 0.11, low_lim = -0.11,opt="G")

POT_jet_prec_95<-evgam(list(excesses~as.numeric(lat)*as.numeric(lon)+month+as.numeric(year)+as.numeric(US_temp_lag2)+as.numeric(jet_strength_lag1)+as.numeric(lat_jet_lag1)+as.numeric(NAO_lag1)+as.numeric(jet_proximity_lag1)+as.numeric(qu_95_prec),~1),family="gpd",da$dat)
R2_POT_jet_prec_95<-Pseudo_R2_point_EVT(POT_jet_prec_95,prec,0.95,0.5,tot_train,tot_test)
p_POT_jet_95_R2<-Plot_R2(R2_POT_jet_prec_95,0.95,up_lim = 0.62)
Pseudo_R2_jet_95_QGAM_POT_prec<-Pseudo_R2_tot_multi_EVT(pred_q_jet_prec_95,POT_jet_prec_95,prec,0.95,0.5,tot_train,tot_test)

R2_multi_POT_jet_R2_95<-Pseudo_R2_point_multi_EVT(pred_q_jet_prec_95,POT_jet_prec_95,prec,0.95,0.5,tot_train,tot_test)
p_q_jet_95_R2_prec<-Plot_R2(R2_multi_POT_jet_R2_95,0.95,up_lim = 0.11, low_lim = -0.11,opt="G")

wind<-annotate_figure(ggarrange(p_q_jet_95_R2_wind+labs(title="95th quantile")+
                                  theme(axis.title.x = element_blank(),
                                                         axis.title.y = element_blank(),legend.key.size = unit(1.5, 'cm')),
                               p_q_jet_99_R2_wind+labs(title="99th quantile")+
                                 theme(axis.title.x = element_blank(),
                                                        axis.title.y = element_blank(),legend.key.size = unit(1.5, 'cm')),
                             common.legend = TRUE,legend="bottom",nrow=1,labels=c("a","b")),
                             text_grob("Daily mean wind speed",face="bold", size=20),
                   )

prec<-annotate_figure(ggarrange(p_q_jet_95_R2_prec+labs(title="95th quantile")+
                                  theme(axis.title.x = element_blank(),
                                                          axis.title.y = element_blank(),legend.key.size = unit(1.5, 'cm')),
                                p_q_jet_99_R2_prec+labs(title="99th quantile")+
                                  theme(axis.title.x = element_blank(),
                                                         axis.title.y = element_blank(),legend.key.size = unit(1.5, 'cm')),
                                common.legend = TRUE,legend="bottom",nrow=1,labels=c("c","d")),
                                text_grob("Daily precipitation",face="bold", size=20)
                      )


annotate_figure(
  ggarrange(wind,prec),text_grob("QGAM vs POT performance",face="bold", size=24))


ggsave(file = "Poster Qgam Pot R2.pdf",
       device="pdf",
       path="Pics",
